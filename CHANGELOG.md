...
- Enabled CommitCraft **agent delegate mode** globally via a new `[agent]` table in `config.toml` (`mode = "delegate"`, `strategy = "single"`). When CommitCraft is driven by an AI agent, `commitcraft ai generate/regenerate/merge/release` now emit a prompt bundle instead of calling the Groq API, and the agent returns the message through `commitcraft ai submit` — removing the per-message API latency. Shipped in CommitCraft v0.67.0. Usage: agent-driven commits use it automatically; pass `--no-agent` (or set `mode = "groq"`) to force the Groq pipeline.
- New delegate prompt templates `CommitCraft/prompts/agent_commit.prompt` and `agent_release.prompt` — the unified single-pass prompts delegate mode fills. Auto-generated by CommitCraft and tracked here so the local customizations travel with the dotfiles.
