...
_odoo_template_list() {
    ...
    if [ "$SELECTED_DIR" = "makefile_template" ]; then
        ...
        DB_CONTAINER_NAME=$(docker-compose ps | fzf --header-lines=2 --header='Select the name of: Db Container' --layout=reverse --color='header:italic:underline:red,label:green' --preview-window=hidden| awk '{print $1}')
